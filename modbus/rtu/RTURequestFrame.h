#pragma once
#include <stdint.h>

#include "../../modbus/frame.h"
#include "../../modbus/misc.h"
#include <string.h>

namespace Modbus
{

    class RTURequestFrame : public RequestFrame
    {
    public:
        uint8_t UnitAddr = 1;			//is the device address encoded as single byte.
        uint16_t CRC = 0;
        uint16_t GetLength()
        {
            if (Message != NULL)
            {
                uint16_t msgLength = Message->GetLength();
                return sizeof(UnitAddr) + sizeof(CRC) + msgLength;
            }
            return 0;
        }


        uint16_t CRC16_2(uint8_t* buf, int len)
        {
            uint16_t crc = 0xFFFF;
            for (int pos = 0; pos < len; pos++)
            {
                crc ^= (uint16_t)buf[pos];    // XOR byte into least sig. byte of crc

                for (int i = 8; i != 0; i--)
                {
                    if ((crc & 0x0001) != 0)
                    {      // If the LSB is set
                        crc >>= 1;                    // Shift right and XOR 0xA001
                        crc ^= 0xA001;
                    }
                    else                            // Else LSB is not set
                        crc >>= 1;                    // Just shift right
                }
            }
            return crc;
        }

        uint16_t CopyTo(uint8_t* data, uint16_t maxLength)
        {
            if (data != NULL && Message != NULL)
            {
                uint16_t totalLength = GetLength();
                if (maxLength >= totalLength)
                {
                    int wrPtr = 0;
                    wrPtr += nmemcpyswapendian(&data[wrPtr], &UnitAddr, sizeof(UnitAddr));
                    wrPtr += Message->CopyTo(&data[wrPtr], maxLength - wrPtr);
                    CRC = CRC16_2(data, wrPtr);
                    wrPtr += nmemcpy(&data[wrPtr], &CRC, sizeof(CRC));
                    return wrPtr;
                }
            }
            return 0;
        }

        uint16_t GetReplyLength()
        {
            if (Message == NULL)
                return 0;
            return Message->GetReplyLength() + 3;
        }
    };
}
